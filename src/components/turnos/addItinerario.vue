    ...
    methods: {
        ...
        ConvertirJson() {
            this.sheetNamesSeleccionado.forEach(sheetName => {
                const {rows,estaciones} = this.obtenerItEstRows(sheetName) as { rows: number[]; estaciones: string[] };
                        ...
                        if (cellTren && typeof cellTren.v === 'number') {
                            ...
                            for(let i = 0; i<=rows.length-1; i++){
                                ...
                                if(cell && typeof cell.v === 'number' && cell.v <= 1){
                                    ...
                                    const totalMinutes = Math.round(cell.v * 24 * 60); // Convertir días a minutos
        ...
        obtenerItEstRows(sheetName:string): { rows: number[]; estaciones: string[] }|undefined{
